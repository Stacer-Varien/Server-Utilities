...
class appealcog(GroupCog, name='appeal'):
    ...
    async def apply(self, ctx: Interaction, warn_id:int):
        ...
        if check_warn == None:
            ...
            try:
                await ctx.followup.send("Please make sure your DMs are opened to start the appealing process", ephemeral=True)

                await ctx.user.send("Please tell us why we should revoke your warn. Please supply video or images if required.\nYou have 3 minutes to tell")
                ...
                try:
                    ...
                    appeal.add_field(name="How to approve or deny?", value=f"{reason}\nWarned by: {moderator}")
