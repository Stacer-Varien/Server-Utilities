...
class appealcog(Cog):
    ...
    async def appeal(self, ctx: Interaction, warn_id):
        ...
        cur = db.cursor()
        cur.execute(
            f"SELECT * FROM warnDATA WHERE user_id = {ctx.user.id} AND warn_id = {warn_id}")
        record = cur.fetchone()
        ...
        if record == None:
            ...
            try:
                aprocess = await ctx.send(f"Please make sure your DMs are opened to start the appealing process",
                                          delete_after=5.0)

                await ctx.user.send(
                    "Please tell us why we should revoke your warn. Please use links instead of images and vidoes if needed.\nYou have 3 minutes to tell")
                ...
                if verdict == "Approve":
                    ...
                    try:
                        ...
                        await ctx.followup.send(
                            "Do you have anything to say why it won't be revoked? Type 'Y' for yes and 'N' for no.")
                        ...
                        if 'Y' in confirm.content:
                            await ctx.followup.send(
                                "Please type what you have to say why it won't be revoked.\nYou have 60 seconds")
