import asyncio
from discord import Forbidden, Object, app_commands as serverutil, Embed, Color
...
class AppealCog(GroupCog, name="appeal"):
    ...
    async def start_appeal(self, ctx, warn_id):
        check_warn = Adwarn().check_id(ctx.author, warn_id)
        ...
        try:
            ...
            try:
                ...
                appeal = Embed(title=f"{ctx.author}'s appeal")
                ...
                await ctx.author.send("You have run out of time. Please try again.")
        except Forbidden:
            await ctx.followup.send(
                "Please open your DMs to start the appeal process", ephemeral=True
            )

    async def approve_appeal(self, ctx, member, warn_id):
    ...
    async def apply(self, ctx, warn_id):
    ...
    async def approve(self, ctx, member, warn_id):
        await ctx.response.defer()
