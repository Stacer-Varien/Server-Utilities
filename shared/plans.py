...
class plancog(GroupCog, name="plan"):
    ...
    @Serverutil.describe(
        ends="Example: 1m (1 minute), 3w (3 weeks), 2y (2 years)")
    @Serverutil.checks.has_any_role(planmanager, operationmanager,
                                    loa_staff_team)
    async def add(self, ctx: Interaction, member: Member, ends: str,
                  plan: str):
        ...
        Plans(guild).add_plan(member, round(ending_time.timestamp()), plan,
                              ctx.user, plan_id)
        ...
        if ctx.guild.id == 925790259160166460:
            ...
            await ctx.followup.send("Plan added to {}".format(
                ha_planlog.mention),
                                    ephemeral=True)
        else:
