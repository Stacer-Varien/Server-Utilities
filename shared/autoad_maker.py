    ...
    async def manage(self, ctx:Interaction, autoad_name:str):
        ...
        if check == True:
            ...
            if view.value=='start':
                ...
                await ctx.edit_original_response(content=f"Autoad {autoad_name} has started", view=None)
            elif view.value == 'stop':
                await self.bot.unload_extension(f"autopost.{autoad_name}")
                await ctx.edit_original_response(content=f"Autoad {autoad_name} has stopped", view=None)
                ...
                await ctx.edit_original_response(
                    content=f"Autoad {autoad_name} has been removed", view=None)
            elif view.value == 'change_ad':
                await ctx.edit_original_response("Provide the ad in a text file and send it back. You have 6 minutes")
