...
class Strike_Appeal(ui.Modal):
    def __init__(self, bot):
        ...
        self.department = ui.TextInput(label="Which department do you work for?", min_length=2, max_length=50,
                                       placeholder="Enter the department here. Remember it has to be the exact were you were striked",
                                       required=True)
    ...
    async def callback(self, ctx: Interaction) -> None:
        ...
        db.execute("INSERT OR IGNORE INTO strike_appeal_data (user_id, strike_appeal_id, department) VALUES (?,?,?)",
                   (ctx.user.id, strike_id, self.department.value))

        embed = Embed(title="New Strike Appeal").add_field(name="Staff member",
                                                           value="{} | {}".format(ctx.user, ctx.user.id),
                                                           inline=False).add_field(name="Department",
                                                                                   value=self.department.value,
                                                                                   inline=True).add_field(
            name="Reason of Appeal", value=self.strike_appeal.value, inline=False).add_field(name="Strike Appeal ID",
                                                                                             value=strike_id,
