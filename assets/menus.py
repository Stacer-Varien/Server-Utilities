...
class PrivacyMenu(ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Select a server", max_values=1, min_values=1, options=options
        )
    ...
    async def callback(self, ctx: Interaction):
        ...
        if self.values[0] == "Orleans":
            ...
            embed.add_field(
                name="Functions",
                value="The channel, <#1041309643449827360> is checked for media content and prohibits members to chat in that channel by deleting that message. If a media content has a message content, the bot will allow it.",
                inline=False,
            )
...
Data is removed if an adwarn has been cleared, breaks approved or denied, end of plans, cleared strike and/or approved or denied resignation.
            ...
            embed.add_field(name="Functions", value="N/A", inline=False)
            await ctx.response.edit_message(embed=embed)
