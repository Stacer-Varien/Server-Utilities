...
class partner(Cog):
    ...
    async def haze(self, ctx: Interaction):
        ...
        try:
            ...
            await ctx.user.send(
                "We have given you our ad. Now, you have 3 minutes to post our ad to your server and send a FULL screenshot as proof. This is just to avoid partnership scams")
        ...
        try:
            ...
            try:
                ...
                request = Embed(title=f"Partnership for HAZE Advertising from {ctx.user}",
                                description="To approve or deny the partnership, use `/partner approve ID APPLIER` or `/partner deny ID REASON`",
                                color=Color.blue())
                request.set_footer(
    ...
    async def approve(self, ctx: Interaction, server=SlashOption(choices=['HAZE', 'HAZE Advertising'], required=True),
                      partner_id=SlashOption(required=True)):
        await ctx.response.defer()
        partnerchannel = self.bot.get_channel(981877384192094279)
