...
class staff_mngm(Cog):
    ...
    async def give(self, ctx: Interaction, member: Member = SlashOption(required=True), department=SlashOption(choices=["Core Team", "Management", "Human Resources", "Moderation", "Marketing"], required=True), reason=SlashOption(required=True)):
        ...
        cur = db.execute(
            "INSERT OR IGNORE INTO strikeData (user_id, strikes, department) VALUES (?, ?, ?)", (member.id, 1, department,))
        ...
        if strikes == 3:
            ...
            embed = Embed(title="You have been striked", color=Color.red()).add_field(name="Strike count", value=strikes,  inline=True).add_field(
                name="Department", value=department, inline=True).add_field(name="Reason", value=reason, inline=True)
            await channel.send(member.mention, embed=embed)
            await ctx.followup.send("Strike given to {}".format(member))
    ...
    async def __approve__(self, ctx: Interaction, member: Member = SlashOption(required=True), department=SlashOption(required=True), kick=SlashOption(description="Only use it if the member is planning on a full resignation (leaving the staff team)", choices=["True", "False"], required=False)):
        await ctx.response.defer(ephemeral=True)
    ...
    @has_any_role(core_team, chr, coo, team_leader, staff_supervisor)
    async def verdict(self, ctx:Interaction, strike_appeal_id=SlashOption(required=True), appealled=SlashOption(name="verdict?", choices=["accept", "deny"], required=True), reason=SlashOption(required=False)):
        await ctx.response.defer()
        user = db.execute("SELECT * FROM strikeData WHERE user_id = ? and department = ?", (ctx.user.id, department,)).fetchone()

        if user == None:
            await ctx.followup.send("Invalid user ID passed", delete_after=5)

        elif user[2] != department:
            await ctx.followup.send("Invalid department", delete_after=5)
