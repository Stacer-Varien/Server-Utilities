...
class plancog(Cog):
    ...
    async def add_plan(self, ctx: Interaction, member: Member = SlashOption(required=True),
        ...
        db.execute(
            ...
            (claimer.id, round(today.timestamp()), round(ending_time.timestamp()), plan, claimee.id, plan_id,
             ctx.guild.id,))
